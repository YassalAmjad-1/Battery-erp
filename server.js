...
// Complete Frontend - Your EXACT Final ERP
app.get('/', (req, res) => {
    const fs = require('fs');
...
<script>
    ...
    async function loadData() {
        try {
            ...
            render();
        } catch(e) { console.error('Error:', e); setTimeout(() => loadData(), 2000); }
    }
    ...
    function calculateDailyTarget(dept, s1800, s2500) {
        ...
        if (dept === 'Acid Dilution') return { 'SG 1.080': (s1800*22.39)+(s2500*20.16), 'SG 1.320': (s1800*23)+(s2500*20.30), 'SG 1.400': (s1800*2.75)+(s2500*4.10) };
        if (dept === 'Molding') {
            let result = {};
    ...
    function getCumulative(dept) { 
        const today = getToday(); let total = 0; 
        for (let e of productionData) if (e.date === today && e.department === dept) { 
    ...
    function getCumulativeForStream(dept, stream) {
        let total = 0;
        for (let e of productionData) if (e.date === getToday() && e.department === dept && e.subEntries) {
    ...
    function getProductionInRange(startDate, endDate) {
        let total = 0;
        for (let e of productionData) if (e.date >= startDate && e.date <= endDate) {
            if (e.subEntries) for (let s of e.subEntries) total += s.actualQty || 0;
            else if (e.actualQtyTX1800 !== undefined) total += (e.actualQtyTX1800 || 0) + (e.actualQtyTX2500 || 0);
            else total += e.actualQty || 0;
        }
    ...
    function endDay() { showMessage('✅ Day ended', 'green'); }
    ...
    function renderProductionForm() {
        ...
        return '<div class="max-w-4xl mx-auto"><div class="bg-green-50 rounded-xl p-4 mb-5"><div class="flex justify-between"><span class="font-bold">' + formDept + '</span><span class="text-lg font-bold text-green-600">Hourly Production Entry</span></div>' + targetDisplay + '</div><div class="bg-white rounded-xl p-5"><form onsubmit="submitEntry(event); return false;"><div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3"><div><label class="text-xs">Department</label><select id="formDept" class="w-full border rounded p-2 text-sm" onchange="updateFormDept(this.value)">' + DEPARTMENTS.map(d=>'<option ' + (formDept===d?'selected':'') + '>' + d + '</option>').join('') + '</select></div><div><label class="text-xs">Shift</label><select id="formShift" class="w-full border rounded p-2 text-sm" onchange="updateFormShift(this.value)">' + SHIFTS.map(s=>'<option ' + (formShift===s?'selected':'') + '>' + s + '</option>').join('') + '</select></div><div><label class="text-xs">Hour (1-hr slot)</label><select id="formHour" class="w-full border rounded p-2 text-sm" onchange="updateFormHour(parseInt(this.value))">' + HOUR_SLOTS.map(h=>'<option value="' + h + '" ' + (formHour===h?'selected':'') + '>' + h + ':00</option>').join('') + '</select></div><div><label class="text-xs">Machine Status</label><select id="machineStatus" class="w-full border rounded p-2 text-sm" onchange="updateMachineStatus(this.value)"><option ' + (machineStatus==='Running'?'selected':'') + '>Running</option><option ' + (machineStatus==='Downtime'?'selected':'') + '>Downtime</option><option ' + (machineStatus==='Maintenance'?'selected':'') + '>Maintenance</option></select></div></div>' + (machineStatus!=='Running'?'<div class="grid grid-cols-2 gap-3 mb-3"><div><label class="text-xs">Downtime (min)</label><input type="number" id="downtimeMins" class="w-full border rounded p-2 text-sm" value="' + downtimeMins + '" onchange="updateDowntimeMins(parseInt(this.value))"></div><div><label class="text-xs">Reason</label><select id="downtimeReason" class="w-full border rounded p-2 text-sm" onchange="updateDowntimeReason(this.value)"><option value="">Select</option>' + DOWNTIME_REASONS.map(r=>'<option ' + (downtimeReason===r?'selected':'') + '>' + r + '</option>').join('') + '</select></div></div>':'') + '<div class="bg-slate-50 p-3 rounded mb-3"><label class="text-xs font-medium block mb-1">📝 Actual Production This Hour</label>' + actualFields + '</div><button type="submit" class="w-full bg-blue-600 text-white py-2 rounded font-semibold text-sm">✅ Submit Entry</button></form></div></div>';
    }

    function renderAnalytics() {
        setTimeout(() => initCharts(), 100);
        return '<div class="flex justify-between items-center mb-3"><h2 class="font-bold">Analytics Dashboard</h2><div class="flex gap-2"><input type="date" id="analyticsStart" value="' + dateRangeStart + '" class="border rounded p-1 text-sm"><input type="date" id="analyticsEnd" value="' + dateRangeEnd + '" class="border rounded p-1 text-sm"><button onclick="updateAnalyticsRange()" class="bg-blue-600 text-white px-2 py-1 rounded text-sm">Apply</button><button onclick="setQuickRange(7)" class="bg-gray-500 text-white px-2 py-1 rounded text-sm">7D</button><button onclick="setQuickRange(30)" class="bg-gray-500 text-white px-2 py-1 rounded text-sm">30D</button></div></div><div class="space-y-4"><div class="chart-container"><h3 class="font-bold text-sm mb-2">📈 Production Trend (' + dateRangeStart + ' to ' + dateRangeEnd + ')</h3><div class="chart-wrapper"><canvas id="weeklyChart"></canvas></div></div><div class="grid grid-cols-1 lg:grid-cols-2 gap-4"><div class="chart-container"><h3 class="font-bold text-sm mb-2">⚡ Department Efficiency (7-Day)</h3><div class="chart-wrapper"><canvas id="efficiencyChart"></canvas></div></div><div class="chart-container"><h3 class="font-bold text-sm mb-2">🏭 OEE by Department</h3><div class="chart-wrapper"><canvas id="oeeChart"></canvas></div></div></div><div class="grid grid-cols-1 lg:grid-cols-2 gap-4"><div class="chart-container"><h3 class="font-bold text-sm mb-2">⏱️ Downtime Analysis (Hours)</h3><div class="chart-wrapper"><canvas id="downtimeChart"></canvas></div></div><div class="chart-container"><h3 class="font-bold text-sm mb-2">🕐 Today\'s Hourly Production Pattern</h3><div class="chart-wrapper"><canvas id="hourlyPatternChart"></canvas></div></div></div></div>';
    }

    function renderInventoryIntelligence() {
        return '<div class="space-y-4"><h2 class="text-lg font-bold">📊 Inventory Management</h2><div class="bg-white rounded-xl shadow p-4"><p class="text-slate-600">Inventory tracking is active. Data is saved to cloud.</p></div></div>';
    }
    ...
    function renderHistory() {
        ...
        for (let e of paginated) {
            ...
            rows += '<tr class="border-b"><td class="p-2 text-xs">' + e.date + '</td><td class="p-2 text-xs">' + e.hourSlot + ':00</td><td class="p-2 text-xs">' + e.department.split(' ')[0] + '</td><td class="p-2 text-xs">' + e.shift + '</td><td class="p-2 text-xs">' + (typeof actual === 'string' ? actual.substring(0,30) : actual) + '</td><td class="p-2"><span class="px-1 py-0.5 rounded-full text-xs ' + (e.machineStatus==='Running'?'bg-green-100 text-green-700':'bg-red-100 text-red-700') + '">' + (e.machineStatus==='Running'?'✅':'⚠️') + '</span></td></tr>';
        }
